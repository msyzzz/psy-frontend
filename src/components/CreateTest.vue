    ...
    <div id="create-test" style="margin-left: 15px">
        ...
        <div v-for="(item, i) in questionnaire.question" :key="i" style="margin-top: 10px">
            ...
            <el-row type="flex" align="middle" v-for="(option, j) in item.option_list" :key="j" style="font-size: 14px">
                ...
                <el-button v-if="j == item.option_list.length - 1" type="primary" round @click="addOption(i)">新增选项</el-button>
                <el-button v-if="j == item.option_list.length - 1 && item.option_list.length > 1" type="primary" round @click="deleteOption(i)">删除选项</el-button>
            </el-row>
            <el-row>
                <el-button v-if="i == this.questionnaire.question.length - 1" type="primary" round @click="addQuestion">新增问题</el-button>
                <el-button v-if="i == this.questionnaire.question.length - 1 && this.questionnaire.question.length > 1" type="primary" round @click="deleteQuestion">删除问题</el-button>
            </el-row>
        </div>
        <el-row type="flex" align="middle" v-for="(item, i) in this.questionnaire.result" :key="i" style="font-size: 14px">
            第{{i+1}}个分值段的结果：
            ...
            <el-button v-if="i == this.questionnaire.result.length - 1" type="primary" round @click="addResult()">新增分值段</el-button>
            <el-button v-if="i == this.questionnaire.result.length - 1 && this.questionnaire.result.length > 1" type="primary" round @click="deleteResult()">删除分值段</el-button>
        </el-row>
